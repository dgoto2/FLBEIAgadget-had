...
main[[1]]$areafile[[1]] <- 
  ...
  bind_rows(list(schedule %>%
              mutate(mean=watertemp)))   ## this bit could be user defined
...
gadgetstock('hockeystock', run.dir, missingOkay = TRUE) %>% 
  gadget_update('refweight', data = stocks[[imm.stock]][[1]]$refweightfile[[1]] ) %>% ### R is not reading the file correctly
  gadget_update('stock',
                minage = 0,
